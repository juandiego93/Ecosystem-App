...
import { UserDataMockService } from '../mock/user-data.mock.services';
...
export class FakeBackendInterceptor implements HttpInterceptor {
  ...
  constructor(private userDataMockService: UserDataMockService) {
    this.USERS = this.userDataMockService.GetUsersData()
  }
  ...
  intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    return of(null).pipe(mergeMap(() => {
      ...
      if (request.url.endsWith('/api/authenticate/login') && request.method === 'POST') {
        ...
        let found: User = this.USERS.find((user: User) => { return (params.IDNumber === user.IDNumber); });
        if (found) {
